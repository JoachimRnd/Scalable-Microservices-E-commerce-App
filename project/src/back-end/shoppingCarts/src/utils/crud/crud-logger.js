...
const info = (message, data, req) => {
    return new Promise((resolve, reject) => {
        ...
        logger.post(`${gateway}/logger/carts/info`, { message, data, request_time_ms : timeDifference})
            .then((success) => {
                resolve(success);
            })
            .catch((error) => {
...
const error = (message, data, req) => {
    return new Promise((resolve, reject) => {
        const timeDifference = computeTimeDifference(req);
        console.log('Time difference in milliseconds: ', timeDifference);
        logger.post(`${gateway}/logger/carts/error`, { message, data, request_time_ms : timeDifference })
            .then((success) => {
                resolve(success);
            })
            .catch((error) => {
